...
- **Autentisering (klientkod):** anonym inloggning (firebase.auth().signInAnonymously()) tillagd i initFirebase(), ny firebase-auth-compat.js-inkludering. Blockerar inte appen om den misslyckas — redan uppsatta lyssnare återansluter automatiskt med giltig token om/när inloggningen går igenom senare
  - **MANUELLT STEG KRÄVS:** aktivera "Anonymous" under Firebase-konsolen → Authentication → Sign-in method. Kan inte göras via kod härifrån
  - **MANUELLT STEG KRÄVS:** uppdatera databasreglerna från true till auth != null för days, turer, breadcrumbs och config. Att skriva/deploya säkerhetsregler räknas som att ändra säkerhetsinställningar — utanför vad jag utför själv, oavsett hur specifikt det efterfrågas
- **Felhantering:** tap(), removeOne(), undoLast(), deleteDeparture() (båda skrivvägarna), adjustTripVehicle() och doReset() hade alla tysta `.catch(() => {})`. Ny global showAppError()/appErrorBanner (samma mönster som submitDriftstatus) — console.error + synligt rött banner (auto-döljs efter 6 s) oavsett aktiv flik. Optimistisk lokal skrivning är oförändrad — bara synkfelet blir synligt
- **Prestanda/riktad query:** pruneRecentBreadcrumbs (heartbeat-triggad, var 10:e–60:e sekund) läser inte längre in hela datum-noden — ny pruneStaleBreadcrumbsQuery() med orderByChild('ts').endAt(cutoff) hämtar bara de förfallna posterna. pruneAllBreadcrumbs (engångskörning vid appstart) oförändrad — behåller sin fulläsning eftersom den ändå måste räkna upp alla datum-noder
  - **MANUELLT STEG KRÄVS (annars ingen effekt av denna fix):** verifierat direkt mot produktionsdatabasen att orderByChild-frågan ger ETT HÅRT FEL utan `"ts": {".indexOn": true}` under breadcrumbs/$date i databasreglerna — inte bara en varning som är det vanliga för RTDB. Tills indexet finns loggas ett engångsfel och ingen riktad rensning sker (pruneAllBreadcrumbs vid appstart fungerar dock fortfarande som backstop)
- **Cleanup:** navigator.geolocation.clearWatch(geoWatchId) tillagd i samma pagehide/beforeunload-hantering som redan städade heartbeat-intervallet — GPS-bevakningen stängdes tidigare inte av vid sidstängning
- Verifierat utan webbläsare (Chrome-tillägget frånkopplat även denna session): escapeHtml() logiskt testad mot fyra XSS-payloads inklusive den rapporterade kedjan och attribut-utbrytningsvektorn (alla farliga tecken korrekt escapade); orderByChild/endAt-frågans faktiska felbeteende bekräftat via REST mot produktionsdatabasen (testdata städat efteråt); statisk korsreferens av alla nya/ändrade symboler och parentesbalans i samtliga filer
